...
<div class="d-flex align-items-center justify-content-between mb-4">
    <div>
        ...
        <h1 class="h3 mb-1 fw-bold text-dark">Edit Permission</h1>
        <p class="small text-muted mb-0">
            Updating: <span class="fw-medium text-primary">{{ $permission->name }}</span>
        </p>
    </div>
...
<div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 1rem;">
    ...
    <div class="card-header border-0 px-4 py-3" style="background: linear-gradient(to right, #f8fafc, #f1f5f9);">
        <div class="d-flex align-items-center gap-3">
            ...
            <div>
                <h5 class="card-title mb-0 fw-bold text-dark" style="font-size: 1rem;">Edit Permission Info</h5>
                <p class="card-text text-muted small mb-0" style="font-size: 0.75rem;">Modify the permission name below.
                </p>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('admin.permissions.update', $permission->id) }}" method="POST">
            ...
            <div class="mb-4">
                <x-form.text name="name" label="Permission Name"
                    placeholder="e.g. role-create, user-delete, setting-view" :value="old('name', $permission->name)" />
                <x-form.text name="display_name" label="Display Name"
                    placeholder="e.g. role-create, user-delete, setting-view"
                    :value="old('name', $permission->display_name)" />
            </div>
