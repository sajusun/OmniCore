@extends('backend.app', ['title' => 'Scan History'])
                ...
                <div class="col-12">
                    <div class="card shadow-sm border-0">
                        <div
                            ...
                            <div>
                                <h3 class="card-title mb-0">All Scan Records</h3>
                            </div>
                        </div>
                        <div class="card-body">
...
<script>
    $(function () {
        let table = $('#scan-history-datatable').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: "{{ route('admin.scan_histories.ajax') }}",
                data: {
                    user: "{{ request('user') }}"
                    }
            },
            columns: [
                ...
                { data: 'score', name: 'ojais_score' },
                { data: 'verdict_label', name: 'verdict_label' },
        ...
        $(document).on('click', '.delete-scan-history', function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('admin.scan_histories.delete', '') }}/' + id,
                        type: 'DELETE',
                        data: {
                            _token: '{{ csrf_token() }}'
                        },
