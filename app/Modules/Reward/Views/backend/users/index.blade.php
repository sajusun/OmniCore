        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                <form method="GET" action="{{ route('admin.rewards.users') }}" class="flex gap-4">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search User Name, Email..." class="flex-1 px-3 py-2 border rounded-md text-sm">
                    <select name="tier_id" class="px-3 py-2 border rounded-md text-sm">
                    ...
                    <a href="{{ route('admin.rewards.users') }}" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md text-sm hover:bg-gray-200">Reset</a>
                </form>
            </div>
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="p-3 font-semibold">
                                    <span class="px-2.5 py-0.5 rounded-full text-xs font-bold text-white" style="background-color: {{ $ur->tier?->color ?? '#6b7280' }}">
                                ...
                                <td class="p-3">
                                    <span class="px-2 py-0.5 bg-amber-50 text-amber-700 rounded text-xs font-semibold">
                                        🔥 {{ $ur->streak_days }} days
                                    </span>
                                </td>
                                <td class="p-3 text-right">
                                    <button onclick="document.getElementById('adjust-modal-{{ $ur->id }}').classList.remove('hidden')" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs rounded font-medium">
                                        Adjust Points
                                    </button>
                                        ...
                                        <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl">
                                            ...
                                            <form method="POST" action="{{ route('admin.rewards.adjust', $ur->user) }}" class="space-y-4">
                                                ...
                                                <div>
                                                    ...
                                                    <input type="number" name="amount" required placeholder="100" class="w-full border rounded-md px-3 py-2 text-sm">
                                                </div>
                                                <div>
                                                    <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Reason / Note</label>
                                                    <input type="text" name="description" required placeholder="e.g. Compensation, Campaign bonus" class="w-full border rounded-md px-3 py-2 text-sm">
                                                </div>
                                                <div class="flex justify-end gap-2 pt-2">
                                                    <button type="button" onclick="document.getElementById('adjust-modal-{{ $ur->id }}').classList.add('hidden')" class="px-4 py-2 bg-gray-100 text-gray-700 rounded-md text-sm">Cancel</button>
