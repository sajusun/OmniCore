...
<div class="container-fluid py-4">
    ...
    <div class="row align-items-center mb-4">
        <div class="col">
            ...
            <p class="text-muted mb-0">Moderate user reviews, ratings, verified buyer feedback, and seller replies.</p>
        </div>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 mb-4">
        <div class="card-body p-3">
            <form method="GET" action="{{ route('admin.reviews.index') }}" class="row g-2 align-items-center">
                <div class="col-md-5">
                    <input type="text" name="search" class="form-control rounded-3" placeholder="Search reviewer, item, or comment content..." value="{{ request('search') }}">
                </div>
                <div class="col-md-2">
                ...
                <div class="col-md-3 d-flex gap-2">
                    ...
                    @if(request()->hasAny(['search', 'status', 'rating']))
                        <a href="{{ route('admin.reviews.index') }}" class="btn btn-outline-secondary rounded-3">
                        ...
                        @forelse($reviews as $review)
                            <tr>
                                        ...
                                        <div class="d-flex gap-1">
                                            @foreach($review->media->take(3) as $m)
                                                <a href="{{ $m->url }}" target="_blank">
                                                    <img src="{{ $m->url }}" class="rounded border" style="width: 40px; height: 40px; object-fit: cover;">
                                                </a>
                                            @endforeach
                                ...
                                <td>
                                    ...
                                    @else
                                        <button class="btn btn-sm btn-outline-info rounded-3" data-bs-toggle="modal" data-bs-target="#replyModal{{ $review->id }}">
                                        ...
                                        <form method="POST" action="{{ route('admin.reviews.reply', $review) }}">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <textarea name="vendor_reply" class="form-control rounded-3" rows="4" required placeholder="Type official response from store or team..."></textarea>
                                                </div>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-secondary rounded-3" data-bs-dismiss="modal">Cancel</button>
                                                <button type="submit" class="btn btn-primary rounded-3">Post Reply</button>
                                            </div>
                                        </form>
                                    </div>
                                </div>
