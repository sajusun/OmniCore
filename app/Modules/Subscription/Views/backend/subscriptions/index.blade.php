<x-admin-layout>
    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <button onclick="document.getElementById('grant-modal').classList.remove('hidden')" class="px-4 py-2 bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white p-4 shadow-sm border border-gray-100">
                <form method="GET" action="{{ route('admin.subscriptions.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search User Name, Email..." class="px-3 py-2 border text-sm">
                    <select name="plan_id" class="px-3 py-2 border text-sm">
                        <option value="">All Plans</option>
                        @foreach($plans as $p)
                        <option value="{{ $p->id }}" {{ request('plan_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                        @endforeach
                    </select>
                    <select name="status" class="px-3 py-2 border text-sm">
                    ...
                    <div class="flex gap-2">
                        ...
                        <a href="{{ route('admin.subscriptions.index') }}" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm hover:bg-gray-200">Reset</a>
                    </div>
                </form>
            </div>
 ...
 {{ $sub->status?->value === 'past_due' ? 'bg-rose-100 text-rose-700' : '' }}">
                                ...
                                <td class="p-3 text-right space-x-2">
                                    <a href="{{ route('admin.subscriptions.show', $sub) }}" class="px-3 py-1 bg-gray-100 hover:bg-gray-200 text-gray-700 text-xs font-medium">
                                        Inspect
                                    </a>
                                    ...
                                    <form method="POST" action="{{ route('admin.subscriptions.cancel', $sub) }}" class="inline" onsubmit="return confirm('Cancel this subscription?')">
                                        @csrf
                                        <button type="submit" class="px-2.5 py-1 text-xs font-medium border border-rose-200 text-rose-600 hover:bg-rose-50">
                                            Cancel
                                        </button>
                                    </form>
                                    @endif
        ...
        <div class="bg-white p-6 max-w-md w-full mx-4 shadow-xl">
            ...
            <form method="POST" action="{{ route('admin.subscriptions.grant') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="user_id" required placeholder="User ID" class="w-full border px-3 py-2 text-sm">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-700 uppercase mb-1">Plan</label>
                    <select name="plan_id" class="w-full border px-3 py-2 text-sm">
                        @foreach($plans as $p)
                ...
                <div class="flex justify-end gap-2 pt-2">
                    <button type="button" onclick="document.getElementById('grant-modal').classList.add('hidden')" class="px-4 py-2 bg-gray-100 text-gray-700 text-sm">Cancel</button>
                    <button type="submit" class="px-4 py-2 bg-emerald-600 text-white text-sm font-semibold hover:bg-emerald-700">Grant Subscription</button>
                </div>
            </form>
        </div>
    </div>
