...
Route::middleware('permission:notification.send')
    ...
    ->group(function () {
        Route::get('/create', [\App\Http\Controllers\Web\Backend\NotificationController::class, 'create'])->name('create');
        Route::post('/store', [\App\Http\Controllers\Web\Backend\NotificationController::class, 'store'])->name('store');
    });

// ─── Support Tickets ──────────────────────────────────────────────────────────
Route::middleware('permission:ticket.list|ticket.reply|ticket.close')
    ->prefix('tickets')->name('tickets.')
    ->group(function () {
        Route::get('/', [\App\Http\Controllers\Web\Backend\TicketController::class, 'index'])->name('index');
        Route::get('/{ticket}', [\App\Http\Controllers\Web\Backend\TicketController::class, 'show'])
            ->middleware('permission:ticket.list')
            ->name('show');
        Route::post('/{ticket}/reply', [\App\Http\Controllers\Web\Backend\TicketController::class, 'reply'])
            ->middleware('permission:ticket.reply')
            ->name('reply');
        Route::patch('/{ticket}/close', [\App\Http\Controllers\Web\Backend\TicketController::class, 'close'])
...
Route::prefix('users')->name('users.')->group(function () {
    ...
    Route::get('/status/{id}', [UserController::class, 'status'])
        ->middleware('permission:user.status')
        ->name('status');
});
...
Route::middleware('permission:permission.list|permission.create|permission.edit|permission.delete')
    ->group(function () {
...
// ─── General Settings ─────────────────────────────────────────────────────────
Route::middleware('permission:settings.access')
    ->prefix('setting/general')->name('setting.general.')
    ->group(function () {
...
// ─── System Optimization ──────────────────────────────────────────────────────
Route::middleware('permission:system.optimize')
...
// ─── CMS / Page Content ───────────────────────────────────────────────────────
Route::middleware('permission:cms.manage')
...
Route::middleware('permission:activity.log.view')->group(function () {
    ...
    Route::get('users/{user}/activity-logs', [\App\Http\Controllers\Admin\ActivityLogController::class, 'userIndex'])->name('users.activity-logs');
});
